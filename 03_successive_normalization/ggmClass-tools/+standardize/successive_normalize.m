function [X output] = successive_normalize(X,varargin)
	...
	while ((stop_err > tol) && (k<=n_iter))
		...
		if(k==0)
			...
			case 'sym'
				...
				[Xc_colpolish mu_c sig_c] = standardize(X); 
				Xc_rowpolish = standardize(Xc_colpolish');	
			
				[Xr_rowpolish mu_r sig_r] = standardize(X');	
			...
			case 'sym'
				...
				Xc_colpolish = standardize(X_rowpolish'); 
				Xc_rowpolish = standardize(Xc_colpolish');	
			
				Xr_rowpolish = standardize(X_colpolish');	
				Xr_colpolish = standardize(Xr_rowpolish'); 
	...
	output_path = strcat(varargin, '.csv');
	disp(strcat("Exporting file: ", output_path));
	%qc_path = strcat(varargin, '_qc-measures.csv');

	csvwrite(output_path{1}, X);
	%struct2csv(qc_path{1}, 'output');
